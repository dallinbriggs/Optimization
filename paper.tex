...
When simultaneously optimizing with 3, 5, and 7~kg payloads we get an optimal solution. The solution flight time is at least 42.46~minutes with a payload not greater then 7~kg. The values of the optimal design variables can be seen in table~\ref{table:unbound_results}. 
...
We have demonstrated a optimization of a gas powered variable pitch multirotor aircraft capable of carrying relatively large payloads for long durations.  The optimization produced a design that is feasible according to our analysis.  The design can carry up to 7~kg for at least 42.5~minutes.  This result is far better then the current capability of vehicles on the market.

The optimization analysis makes several assumptions and approximations that would need to be relaxed in order to better trust the solution.  Further the optimization chose a design from continuous variables when only discrete off-the-shelf choices are available. 
